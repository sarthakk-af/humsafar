...
export default function SingleBlogScreen({ navigation, route }) {
    ...
    const fetchComments = async () => {
        try {
            const response = await fetch(`http://10.0.7.255:8000/api/get-comments/${blog._id}`, {
                method: 'GET',
                headers: {
    ...
    const handlePostComment = async () => {
        ...
        try {
            const response = await fetch('http://10.0.7.255:8000/api/post-comment', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    Accept: 'application/json'
                    ...
                    <Swiper style={styles.wrapper} showsButtons={true}>
                        {blog.images.map((image, index) => (
                            <View key={index} style={styles.slide}>
                                <Image source={{ uri: `http://10.0.7.255:8000/${image}` }} style={styles.image} />
                            </View>
                        ))}
                    </Swiper>
